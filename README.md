...
在图片上按住鼠标，拖出 PicSee 窗口后，放到桌面、Finder 文件夹或支持文件拖放的应用中，即可复制当前图片，无需按 Option。窗口内仍保持原来的平移行为；离开图片边缘或进入窗口留白不会导出。鼠标需距按下位置移动至少 12 点，并越过整个窗口外边界至少 12 点，才开始系统文件拖拽。

无标题栏时，从顶部拖动仍然移动窗口，从右下角或左上角拖动仍然调整窗口大小。文字选择、缩略导航图和截图编辑继续使用各自原有的拖动操作。系统文件拖拽开始后，本次手势由系统接管，取消不会修改原图。

本地图片优先使用原始文件，保留文件名、扩展名和原始内容，包括 GIF 动画；查看时的缩放和旋转不会写入副本。临时来源会复制到系统临时目录，只有内存图像可用时会导出为 PNG。临时导出文件在后续启动或导出时清理：保留至少 24 小时，并跳过仍在运行的来源进程，避免接收方尚未读取就被删除。目前查看入口仍为本地文件，尚未增加网络 URL、Blob 或剪贴板图片的导入入口。

全屏或窗口贴住屏幕边缘时，鼠标可能无法越过窗口边界；可先缩小窗口，或使用现有的“图片另存为…”功能。

实现与验证范围见[拖拽保存验证记录](docs/verification/2026-09-13-image-file-drag.md)。
...
- 可在设置页或右键菜单切换“窗口置顶”，默认关闭并在本地记忆。所有已有及新开的看图窗口同步生效，切换应用后仍在普通窗口之上；多个置顶窗口按正常点击顺序前后排列，不抢焦点。换图、最小化后恢复均保留置顶，系统全屏期间暂时恢复普通层级，退出全屏后按当前设置恢复。设置窗口及操作弹窗显示在置顶图片上方，不自动跨桌面跟随。
- 默认多窗口独立查看，关闭一个窗口只退出当前进程
- 可在“设置 → 显示设置 → 窗口”或右键菜单开启“单窗口看图”：后续打开的图片复用接收窗口，保留窗口位置、大小和全屏状态，重置缩放、平移、旋转、OCR 和截图状态，并按新图片所在文件夹重新建立左右翻图列表。重复打开当前图片仅激活窗口；加载失败保留原图；截图编辑中换图需先确认。批量打开只显示第一张，仍按所在文件夹浏览。开关默认关闭，切换时不关闭已有窗口。
...
- 幻灯片：自动播放当前文件夹，支持间隔调整、循环、暂停与继续；窗口和全屏均可播放
- 双击图片进入或退出 macOS 原生全屏
- `Esc` 关闭当前图片并退出应用
- 右键菜单支持复制图片路径、切换跟随系统 / 浅色 / 深色主题
- 含有透明或半透明像素的图片按需显示 Photoshop 风格的灰白棋盘格；普通照片及完全不透明的 PNG 不显示。透明度判断按图片缓存，主图、切图过渡和缩略导航图共用结果。棋盘格仅用于预览，不会写入导出的图片。
- 右键「移到废纸篓」、按 `Delete` 或 `⌘⌫` 删除当前图片，自动切到下一张（末尾则切到上一张）；支持 `⌘Z` 或右键「撤销移到废纸篓」逐次恢复。撤销记录在关闭窗口或单窗口模式成功打开另一张外部图片时清空，恢复时不会覆盖同名文件。删完后显示空文件夹提示，可撤销或退出
- 删除前默认显示确认框，回车或 Escape 取消；确认删除时勾选「以后不再询问」可关闭后续确认，重启应用后仍生效

单窗口与窗口置顶的测试及实测范围见[验证记录](docs/verification/2026-09-26-window-modes.md)。
...
图片立即显示，排序查询在后台执行；完成前的翻页按键会排队，完成后依次处理，避免过早翻页锁定错误的文件名顺序。读取过程不显示提示；查询失败时静默使用名称自然升序，不会在浏览中途自行换序。
...
- 画廊、自动排列的图标视图、无法识别的保存规则，以及不同分组字段和组内排序字段的组合，暂不能保证还原显示顺序，读取失败时静默回退到名称自然升序。
- 已保存的 Finder 视图信息可能滞后，自动读取不保证所有视图和系统版本完全一致。

### 幻灯片播放

点击底部工具栏的播放按钮，或右键选择「播放幻灯片」，从当前图片开始自动播放当前文件夹。沿用当前 Finder 排序，不包含子文件夹，也不会强制进入全屏。排序尚未准备好时先等待，准备好后当前图片仍会展示一个完整间隔。

- 默认每张 **5 秒**，可在播放控制条选择 **2、3、5、10 秒**；播放间隔和循环开关会记住上次选择。
- 默认按当前文件夹顺序循环播放，最后一张之后回到第一张；关闭循环后播放到末尾结束并保留当前图片。循环开关会记住上次选择。
- 播放控制条提供 **全屏播放／退出全屏** 按钮，切换时保持播放或暂停状态，动画完成后重新计时。
- **空格**暂停或继续，**方向键**手动切图并重新计时；暂停时手动切图不会自动恢复播放。
- **Esc** 结束幻灯片并停留在当前图片，再次按 Esc 按原有逻辑退出 PicSee。普通看图时空格的退出行为保持不变。
- 播放时鼠标静止约 2 秒会隐藏控制条和信息浮层，移动鼠标重新显示；指针停留在播放控制条上或暂停时保持显示。隐藏普通工具栏也不影响幻灯片控制条。
- 缩放、平移、选择文字、截图、旋转、删除确认、打开右键菜单或切到其他窗口／应用时会暂停，返回后手动继续。电脑睡眠时也会暂停。
- 自动跳过已移走或无法解码的图片，没有下一张可播放图片时结束。结束后恢复原来的工具栏及信息显示偏好。
...
- 右键选择「移到废纸篓」，或按 `Delete`、前向删除键（`Fn + Delete`）或 `Cmd + Delete`，将当前原文件移到系统废纸篓。长按不会连续删除；编辑文字或使用截图工具时不触发图片删除。
- 默认显示确认框：左侧「取消」、右侧「移到废纸篓」；回车或 `Esc` 取消。确认删除时勾选「以后不再询问」可关闭后续确认，取消操作不会保存这一偏好。
- 删除后继续显示下一张；后面没有可浏览图片时回到上一张。删完后显示空文件夹提示，可撤销或退出。
- 点击成功提示中的「撤销」、右键选择「撤销移到废纸篓」，或按 `Cmd + Z`，可逐次恢复当前看图会话中删除的图片。关闭窗口或单窗口模式成功打开另一张外部图片时清空撤销记录；左右翻图不会清空。恢复时不会覆盖原位置的同名文件。
