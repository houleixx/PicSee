...
- 底部工具条支持截图与标注：直接在当前图片窗口中拖动选择区域，通过八个边角控制点调整大小，拖动选区内部移动；在选区外拖动可再次选择，Esc 退出截图模式
- 截图支持画笔、荧光笔、直线、箭头、矩形、椭圆、文字、马赛克和删除标注，可调整颜色、粗细、字号，支持撤销 / 重做
- 截图工具条按当前工具显示参数：文字显示颜色与字号，马赛克显示圆形画刷直径，画笔和形状显示颜色与粗细；宽、高始终显示在右侧；提供常用颜色、画笔粗细与马赛克直径快捷选项，点击当前粗细数值可打开自定义滑条
- 点击截图工具条的 `T`，再点击选区即可透明输入文字，输入框随内容自动扩展；回车换行，点击框外确认，Esc 取消本次修改；确认后可拖动文字移动，双击重新编辑
- 截图工具条底部的宽、高像素可直接修改，选框实时跟随调整，移动时保持尺寸不变；首次框选不计入撤销，后续标注和调整支持撤销 / 重做
- 截图工具条支持浅色与深色模式；字号、画笔粗细与马赛克直径以当前屏幕显示大小为准，保存时图片与标注统一按所选比例缩放
- 截图可复制到剪贴板或另存为 PNG，并保留当前旋转方向；保存时默认按选中大小输出，也可按原图比例恢复：选中大小乘以图片显示比例的倒数（例如显示为 50% 时系数为 2，显示为 200% 时系数为 0.5），窗口中显示选中大小、缩放系数与计算后的输出像素
- 滚轮缩放图片
- 放大后拖动图片空白区域可平移图片
